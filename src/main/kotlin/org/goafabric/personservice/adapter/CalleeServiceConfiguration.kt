...
class CalleeServiceConfiguration {
    ...
    fun restTemplate(
        ...
        val restTemplate = builder
            ...
            .build()
        restTemplate.setMessageConverters(listOf<HttpMessageConverter<*>>(MappingJackson2HttpMessageConverter()))
        restTemplate.interceptors.add(ClientHttpRequestInterceptor { request: HttpRequest, body: ByteArray?, execution: ClientHttpRequestExecution ->
            ...
            request.headers["X-TenantId"] = HttpInterceptor.getTenantId()
            request.headers["X-Auth-Request-Preferred-Username"] = HttpInterceptor.getUserName()
